...
# Importar librería para hablar con la API de FOG (crear sesiones multicast).
# Igual que con paramiko/scp: si falta, avisa al usar el botón en vez de
...
#  recién clonados (ver _candidatas_clave_ssh, más arriba). Un token de FOG
#  puede crear tareas de imaging sobre cualquier equipo del inventario, así
#  que filtrarlo ahí sería mucho más grave que filtrar la clave SSH de
#  solo-subida. Por eso se leen de un archivo externo (_candidatas_config_fog)
#  que solo existe en la PC central donde se abre el Panel de clonación.
# ============================================================================
...
def abrir_panel_clonacion(ventana_padre):
    ...
    def crear_multicast():
        try:
            ...
            messagebox.showerror("No se pudo conectar a FOG", str(e), parent=win)
            return
        imagenes = sorted(
        ...
        if not imagenes:
            messagebox.showwarning(
                "Sin imágenes",
                "FOG no devolvió ninguna imagen habilitada.",
                parent=win,
            )
            return
        nombres = [i["name"] for i in imagenes]
        por_nombre = {i["name"]: i["id"] for i in imagenes}

        lote = estado["lote"]
        cantidad_sugerida = len(estado["esperados"]) or 1

        dlg = ventana_modal(win, "Crear sesión multicast en FOG", "460x340")
        titulo_ui(dlg, "🛰️ Nueva sesión multicast", size=12)

        frm = tk.Frame(dlg, bg=COLORS["fondo"])
        frm.pack(padx=20, pady=10, fill="x")

        tk.Label(
            frm, text="Nombre de la sesión:", font=fuente(10, True),
            bg=COLORS["fondo"],
        ).grid(row=0, column=0, sticky="e", padx=6, pady=6)
        entry_nombre = tk.Entry(frm, font=fuente(11), width=28)
        entry_nombre.insert(0, etiqueta_lote(lote) if lote else "")
        entry_nombre.grid(row=0, column=1, padx=6, pady=6)

        tk.Label(
            frm, text="Imagen:", font=fuente(10, True), bg=COLORS["fondo"]
        ).grid(row=1, column=0, sticky="e", padx=6, pady=6)
        combo_imagen = ttk.Combobox(
            frm, values=nombres, state="readonly", font=fuente(10), width=26
        )
        combo_imagen.grid(row=1, column=1, padx=6, pady=6)

        tk.Label(
            frm, text="Cantidad de equipos:", font=fuente(10, True),
            bg=COLORS["fondo"],
        ).grid(row=2, column=0, sticky="e", padx=6, pady=6)
        entry_cantidad = tk.Entry(frm, font=fuente(11), width=10)
        entry_cantidad.insert(0, str(cantidad_sugerida))
        entry_cantidad.grid(row=2, column=1, sticky="w", padx=6, pady=6)

        tk.Label(
            dlg,
            text="Los equipos se siguen uniendo a mano por PXE con este "
        ...
        def confirmar():
            ...
            imagen_nombre = combo_imagen.get()
            cantidad_txt = entry_cantidad.get().strip()
            if not nombre:
                messagebox.showwarning(
                    "Falta el nombre", "Ponele un nombre a la sesión.", parent=dlg
                )
                return
            if imagen_nombre not in por_nombre:
                messagebox.showwarning(
                    "Falta la imagen", "Elegí una imagen de la lista.", parent=dlg
                )
                return
            if not cantidad_txt.isdigit() or int(cantidad_txt) < 1:
                messagebox.showwarning(
                    "Cantidad inválida",
                    "La cantidad tiene que ser un número mayor a 0.",
                    parent=dlg,
                )
                return
            try:
                sesion = fog_crear_sesion_multicast(
                    nombre, por_nombre[imagen_nombre], int(cantidad_txt)
                )
            except Exception as e:
                messagebox.showerror(
                    "No se pudo crear la sesión", str(e), parent=dlg
                )
                return
            dlg.destroy()
            messagebox.showinfo(
                "Sesión creada",
                f"'{sesion.get('name')}' arrancó en el puerto "
                f"{sesion.get('port')}.\n\n"
                f"Ahora booteá por PXE los {cantidad_txt} equipo(s) y elegí "
                "esa sesión para unirse, igual que siempre.",
                parent=win,
            )

        boton_accion(dlg, "🚀 Crear sesión", confirmar, COLORS["verde"]).pack(
            pady=14
        )

    frame_btns = tk.Frame(win, bg=COLORS["fondo"])
    frame_btns.pack(pady=(12, 4))
    ...
    # fila de arriba y queda lista para sumar el botón de unicast al lado.
    frame_btns_fog = tk.Frame(win, bg=COLORS["fondo"])
    frame_btns_fog.pack(pady=(0, 12))
    for texto, comando, color in (
        ("🛰️ Multicast en FOG", crear_multicast, COLORS["naranja"]),
    ):
        boton_accion(frame_btns_fog, texto, comando, color).pack(side="left", padx=4)
